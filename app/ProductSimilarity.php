...
class ProductSimilarity
{
    ...
    public function __construct(array $products)
    {
        ...
        $this->priceHighRange = max(array_column($products, 'sale_price'));
    }
...
//                (Similarity::hamming($productAFeatures, $productBFeatures) * $this->featureWeight),
                (Similarity::euclidean(
                        Similarity::minMaxNorm([$productA->sale_price], 0, $this->priceHighRange),
                        Similarity::minMaxNorm([$productB->sale_price], 0, $this->priceHighRange)
                    ) * $this->priceWeight),
                (Similarity::jaccard(strval($productA->category_id), strval($productB->category_id)) * $this->categoryWeight)
