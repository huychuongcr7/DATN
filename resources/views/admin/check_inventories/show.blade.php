...
@section('breadcrumb')
    @include('layouts.backend.breadcrumb', [
        'items' => [
            ...
            'url' => route('admin.check_inventories.index')
            ],
            [
                                ...
                                <table id="basic-datatables" class="display table table-striped table-hover dataTable no-footer">
                                    ...
                                    @foreach($productNews as $productNew)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>
                                                <a class="text-center my-3" href="{{ route('admin.products.show', $productNew['id']) }}">{{ $productNew['name'] }}</a>
                                            </td>
                                            <td>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    @if($checkInventory->status == \App\Models\CheckInventory::STATUS_TEMPORARY)
                                        <a class="btn btn-default" href="{{ route('admin.check_inventories.index') }}">
                                            <span class="btn-label">
                                                <i class="fa fa-arrow-left"></i>
                                            </span>Quay lại
                                        </a>
                                        <a class="btn btn-primary" href="{{ route('admin.check_inventories.edit', $checkInventory->id) }}">
                                            <span class="btn-label">
                        ...
                        <div class="modal fade" id="{{ 'balanceModal' }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form class="form-group" method="POST" action="{{ route('admin.check_inventories.balance', $checkInventory->id) }}">
                                            @csrf
                                            @method('PUT')
